...
import { Send, Square, Plus, Paperclip, X, Copy, Check, Terminal, Code2, Image as ImageIcon, Search, ListTree, MessageSquare, Lightbulb, Trash2, Folder, ArrowLeft, ChevronDown, PanelLeftClose, PanelLeftOpen, Maximize2, Minimize2 } from "lucide-react";
...
export default function ProjectPage() {
  ...
  const { projects, updateProject, folders, chats, createChat, setActiveChat, addMessage, updateLastMessage, deleteChat, vault, settings } = useGodEye();
  ...
  const activeChat = projectChats.find(c => c.id === activeChatId) || projectChats[0] || null;
  ...
  const folderChain = useMemo(() => {
    if (!project?.folderId) return [] as any[];
    const chain: any[] = [];
    let cur: any = folders.find(f => f.id === project.folderId);
  ...
  // sync mode/provider/model when active chat changes
  useEffect(() => {
    if (activeChat) {
  ...
  }, [activeChatId, projectChats.length]);

  // auto-create first chat when opening a project
  useEffect(() => {
    if (project && projectChats.length === 0) {
      const id = createChat({ mode, provider, model, projectId });
      setLocalChat(id);
    }
  }, [project?.id, projectChats.length]);
        ...
        <div className="border-b bg-background/80 backdrop-blur shrink-0">
          <div className="px-4 md:px-6 pt-3 pb-3">
            <div className="flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
              ...
              {folderChain.map((f, i) => (
                <span key={f.id} className="inline-flex items-center gap-1.5">
                  <span>/</span>
                  <Link href={`/dashboard?folder=${f.id}`} className="inline-flex items-center gap-1 hover:text-foreground"><Folder className="h-3 w-3" style={{ color: f.color }} /> {f.name}</Link>
                </span>
              ))}
              <span>/</span>
            ...
            <div className="mt-2 flex flex-wrap items-center gap-3">
              <div className="h-9 w-9 rounded-xl grid place-items-center text-white text-sm shrink-0" style={{ background: project.color }}>{project.name.slice(0, 1).toUpperCase()}</div>
              <div className="min-w-0">
                <div className="font-semibold text-sm md:text-base truncate">{project.name}</div>
                <div className="text-xs text-muted-foreground truncate">{project.description}</div>
              </div>
              <span className={`text-[11px] rounded-full px-2 py-1 border capitalize shrink-0 ${project.status === "active" ? "bg-emerald-50 border-emerald-200 text-emerald-700" : project.status === "draft" ? "bg-amber-50 border-amber-200 text-amber-700" : "bg-muted"}`}>{project.status}</span>
              <div className="flex items-center gap-2 ml-auto shrink-0">
                <button onClick={() => updateProject(project.id, { tasksDone: Math.max(0, project.tasksDone - 1), tasksTotal: Math.max(project.tasksDone, project.tasksTotal - 1) })} className="p-1.5 rounded-full border bg-card hover:bg-muted" title="Decrease task"><X className="h-3.5 w-3.5" /></button>
                <span className="text-xs text-muted-foreground">{project.tasksDone}/{project.tasksTotal} tasks</span>
                <button onClick={() => updateProject(project.id, { tasksDone: project.tasksDone + 1, tasksTotal: project.tasksTotal + 1 })} className="p-1.5 rounded-full border bg-card hover:bg-muted" title="Add task"><Plus className="h-3.5 w-3.5" /></button>
                <div className="hidden sm:block h-1.5 w-24 rounded-full bg-muted overflow-hidden"><div className="h-full" style={{ width: `${pct}%`, background: project.color }} /></div>
              </div>
            </div>
          </div>
        </div>
          ...
          {mobileRailOpen && (
            <aside className="fixed inset-y-0 left-0 z-50 flex w-[85vw] max-w-[280px] flex-col border-r bg-card md:hidden">
              <div className="p-3 border-b flex items-center justify-between">
                <div className="font-semibold text-xs uppercase tracking-wider text-muted-foreground">Project chats</div>
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-1">
                {projectChats.length === 0 && <div className="text-xs text-muted-foreground p-3">No chats yet — start one.</div>}
                {projectChats.map(c => (
          ...
          <section className="flex-1 min-w-0 flex flex-col">
            ...
            <div className="h-14 border-b flex items-center gap-2 px-3 md:px-4 bg-background/80 backdrop-blur shrink-0">
              <div className="flex items-center gap-1.5 shrink-0">
                <button onClick={() => setMobileRailOpen(!mobileRailOpen)} className="md:hidden p-2 rounded-xl border bg-card hover:bg-muted" title="Project chats">
                  {mobileRailOpen ? <PanelLeftClose className="h-4 w-4"/> : <PanelLeftOpen className="h-4 w-4"/>}
                </button>
                <button onClick={() => setRailHidden(!railHidden)} className="hidden md:inline-flex p-2 rounded-xl border bg-card hover:bg-muted" title={railHidden ? "Show project chats" : "Hide project chats"}>
            ...
            <div ref={listRef} className="flex-1 min-h-0 overflow-y-auto p-4 md:p-6 space-y-4 overscroll-contain">
              {!activeChat || activeChat.messages.length === 0 ? (
                <div className="max-w-2xl mx-auto text-center py-16">
                  <div className="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs"><span className="h-2 w-2 rounded-full" style={{ background: project.color }} /> {project.name} workspace</div>
                  <h2 className="mt-4 text-2xl font-semibold">What should GodEye build for {project.name}?</h2>
                  <p className="text-sm text-muted-foreground mt-1">Every message stays saved in this project — upgrades, fixes, ideas, all in one place.</p>
                </div>
              ...
              <div className="max-w-3xl mx-auto">
                <div className="rounded-2xl border bg-background p-2 shadow-sm">
                  <div className="flex items-end gap-2">
                    ...
                    <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); if (!running) send(); } }} placeholder={`Message ${project.name} • ${mode} mode • Shift+Enter for newline`} rows={1} className="flex-1 min-w-0 bg-transparent outline-none text-sm resize-none py-2 max-h-32" />
                ...
                <div className="mt-2 flex items-center justify-between text-[11px] text-muted-foreground">
                  <span className="hidden sm:inline">Saved to this project • Enter to send • Shift+Enter newline</span>
                  <span className="sm:hidden">Saved to this project</span>
